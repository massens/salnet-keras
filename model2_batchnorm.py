...
def conv_bn_relu(n, k, name=None, border_mode='same', **kwargs):
    conv = Convolution2D(
        n, k, k,
        activation='linear',
...
def get_model():
    ...
    x = conv_bn_relu( 512, 5,  'conv4_n')(x)
    x = conv_bn_relu( 512, 5,  'conv5_n')(x)
    x = conv_bn_relu( 256, 7,  'conv6_n')(x)
    x = conv_bn_relu( 128, 11, 'conv7_n')(x)
    x = conv_bn_relu( 32,  11, 'conv8_n')(x)
    x = conv_bn_relu(  1,  13, 'conv9_n')(x)
